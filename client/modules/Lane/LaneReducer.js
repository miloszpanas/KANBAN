import { CREATE_LANE, CREATE_LANES, UPDATE_LANE, DELETE_LANE, EDIT_LANE, MOVE_BEETWEN_LANES, REMOVE_FROM_LANE, PUSH_TO_LANE } from './LaneActions';
...
export default function lanes(state = initialState, action) {
    switch (action.type) {
        ...
        case MOVE_WITHIN_LANE: {
            const NewLane = { ...state[acttion.laneId] };
            newLane.notes = moveNotes(NewLane.notes, action.sourceId, action.targetId);
         
            return { ...state, [action.laneId]: newLane };
         }
         
        case MOVE_BEETWEN_LANES: {
            const targerLane = { ...state[action.targetLaneId]};
            targerLane.notes = [ ...targetLane.notes, action.noteId];
            
            const sourceLane = { ...state[action.sourceLaneId] };
            sourceLane.notes = sourceLane.notes.filter(noteId !== action.noteId);
        ...
        case PUSH_TO_LANE: {
            ...
            return { ...state, [action.targetLaneId]: targretLane };
        }
